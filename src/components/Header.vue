            ...
            <div id="carrito" class="bg-white p-3">
              ...
              <div v-else>
                <table class="w-100 table">
                  ...
                  <tbody>
                    <tr v-for="pedido in carrito" :key="pedido.id">
                      <td>
                        <img
                          class="img-fluid"
                      ...
                      <td>{{ pedido.nombre }}</td>
                      <td class="fw-bold">€{{pedido.precio}}</td>
                      <td class="flex align-items-start gap-4">
                        <button 
                          type="button" 
                          class="btn btn-dark"
                          @click="$emit('restar-producto', pedido.id)"
                        >
                        -
                        </button>
                        {{pedido.cantidad}}
                        <button 
                          type="button" 
                          class="btn btn-dark"
                          @click="$emit('sumar-producto', pedido.id)"
                        >
                        +
                        </button>
                      </td>
                      <td>
                        <button 
                          ...
                          type="button"
                          @click="$emit('eliminar-producto', pedido.id)"
                        >
                        X
                        </button>
                      </td>
...
<script setup>
  ...
  defineEmits(['sumar-producto', 'restar-producto','agregar-carrito', 'eliminar-producto','vaciar-carrito'])
